setwd("C:/Users/Prasanta/Downloads/PGPBABI/Hackathon/McKinseyHealthCare")
...
traindfFinal <- merge(traindf1,miceOutput,by="id")
View(traindfFinal)
traindfFinal <- traindfFinal[,-c(3)]
...
traindfFinal <- traindfFinal[,-c(3,5,11,12)]
...
write.csv(final,"sample_submission5.csv")

write.csv(testdf,"sample_submission4.csv")
...
#trainSMOTE2 <- trainSMOTE[,-c(13,14)]
...
#data1 <- rbind(trainSMOTE2,testdf2)
traindfFinal$smoking_status <- as.character(traindfFinal$smoking_status)
...
#testdf2$smoking_status <- as.character(testdf2$smoking_status)
